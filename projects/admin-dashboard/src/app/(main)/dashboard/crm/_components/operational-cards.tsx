  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs sm:grid-cols-2 xl:grid-cols-3">
      <Card>
        ...
        <CardFooter>
          <p className="text-muted-foreground text-xs">自上月以来线索增长18.2%。</p>
        </CardFooter>
      </Card>

      <Card>
        <CardHeader>
          <CardTitle>按地区划分的销售</CardTitle>
        ...
        <CardFooter>
          <div className="flex justify-between gap-1 text-muted-foreground text-xs">
            <span>跟踪{regionSalesData.length}个地区</span>
            <span>•</span>
            <span>{regionSalesData.filter((r) => r.isPositive).length}个地区增长中</span>
          </div>
        </CardFooter>
      </Card>
