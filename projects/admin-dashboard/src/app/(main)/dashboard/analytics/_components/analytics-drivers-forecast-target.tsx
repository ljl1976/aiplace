...
export function DriversForecastTarget() {
  ...
  return (
    <Card className="shadow-xs">
      <CardHeader>
        <CardTitle>预测与目标对比</CardTitle>
        <CardDescription>12周趋势及达成情况</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <MetricChip label="达成率" value="72.4%" note="已成交/月度目标" />
          <MetricChip label="加权管道" value="$1,284,000" note="对比剩余$668,000" />
          <MetricChip label="预测置信度" value="81.0%" note="波动率调整后的置信度" />
        </div>
